...
function PushPullGithub(form_results) {
    ...
    $.ajax({
        type: "GET",
        url: "https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season43/Season43_Responses.json",
        dataType: "json",
            ...
            api.setBranch('master')
                .then( () => api.pushFiles(
                    ...
                    [{
                        ...
                        path: 'docs/Season43/Season43_Responses.json'
                    }]
                ))
                .then(function() {
                    console.log('Files committed!');
                    window.location = "http://ethanebinger.com/Fantasy-Survivor/Season43/results.html"
                });
		}
	});
};
...
function init_chart() {
    ...
    $.when(
        ...
        $.get("https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season43/Season43_Responses.json", function(result) {
...
function determineWeek(responses) {
    ...
    if (submit_time <= new Date(2022,8,25,0)) { // September 21, 2022
        ...
        cur_vote = 5;
    } else if (submit_time <= new Date(2022,9,30,0)) { // October 26, 2022
        cur_vote = 6;
...
function final_eight_calc(scores, result) {
    ...
    function which_castaway(castaways){
        var sum = 0,
			bonus = 0;
        for (var i=1; i<9; i++){
            if (castaways['place_'+String([i])] === "Gabler") {			// sole survivor
                sum += Math.pow(Math.abs(i-1),2.25);
				if (i===1) { bonus += 5 };
            } else if (castaways['place_'+String([i])] === "Cassidy") {	// runner up
                sum += Math.pow(Math.abs(i-2),2.25);
				if (i===2) { bonus += 5 };
            } else if (castaways['place_'+String([i])] === "Owen") {	// third
                sum += Math.pow(Math.abs(i-3),2.25);
				if (i===3) { bonus += 5 };
            } else if (castaways['place_'+String([i])] === "Jesse") {	// fourth
                sum += Math.pow(Math.abs(i-4),2.25);
				if (i===4) { bonus += 5 };
            } else if (castaways['place_'+String([i])] === "Karla") {	// fifth
                sum += Math.pow(Math.abs(i-5),2.25);
				if (i===5) { bonus += 5 };
            } else if (castaways['place_'+String([i])] === "Cody") {	// sixth
...
function final_three_calc(scores, result) {
    var top_three = ['Gabler', 'Owen', 'Cassidy'];  //top three!
	var name_ep_count = [0];
	for (var n=0; n<scores.length; n++) {
        for (var i=0; i<result.length; i++) {
			if (result[i].name === scores[n].name) {
