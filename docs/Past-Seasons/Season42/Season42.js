...
function PushPullGithub(form_results) {
    ...
    $.ajax({
        type: "GET",
        url: "https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season42/Season42_Responses.json",
        dataType: "json",
            ...
            api.setBranch('master')
                .then( () => api.pushFiles(
                    ...
                    [{
                        ...
                        path: 'docs/Season42/Season42_Responses.json'
                    }]
                ))
                .then(function() {
                    console.log('Files committed!');
                    window.location = "http://ethanebinger.com/Fantasy-Survivor/Season42/results.html"
                });
		}
	});
};
...
function init_chart() {
    ...
    $.when(
        ...
        $.get("https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season42/Season42_Responses.json", function(result) {
...
function determineWeek(responses) {
    ...
    if (submit_time <= new Date(2022,2,13,0)) {
        ...
        cur_vote = 10;
    } else if (submit_time <= new Date(2022,4,14,0)) {
        cur_vote = 11;
    } else if (submit_time <= new Date(2022,4,21,0)) {
        cur_vote = 12;
    } else if (submit_time <= new Date(2022,5,7,0)) {
        cur_vote = 12;
    };
    return cur_vote;
};
...
var results = [
	...
	{	'vote': 3,
        ...
        'idolPlayed': 'No',
        'titleQuote': 'Omar',
		'summit': ['Chanelle', 'Omar'],
        'nudity': 'No',
        'team_yellow': ['Jonathan', 'Lindsay', 'Maryanne', 'Omar'],
        'team_blue': ['Drea', 'Rocksroy', 'Romeo', 'Swati', 'Tori'],
		'team_green': ['Chanelle', 'Daniel', 'Jenny', 'Hai', 'Lydia', 'Mike']
    },
	{	'vote': 4,
        'date': '3/30/22',
        'merge': 'No',
