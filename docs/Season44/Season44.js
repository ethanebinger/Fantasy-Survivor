...
function PushPullGithub(form_results) {
    ...
    $.ajax({
        type: "GET",
        url: "https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season44/Season44_Responses.json",
        dataType: "json",
            ...
            api.setBranch('master')
                .then( () => api.pushFiles(
                    ...
                    [{
                        ...
                        path: 'docs/Season44/Season44_Responses.json'
                    }]
                ))
                .then(function() {
                    console.log('Files committed!');
                    window.location = "http://ethanebinger.com/Fantasy-Survivor/Season44/results.html"
                });
		}
	});
};
...
function init_chart() {
    ...
    $.when(
        ...
        $.get("https://api.github.com/repos/ethanebinger/Fantasy-Survivor/contents/docs/Season44/Season44_Responses.json", function(result) {
...
function determineWeek(responses) {
    ...
    if (submit_time <= new Date(2022,2,3,0)) { // March 1, 2023
        cur_vote = 1;
    } else if (submit_time <= new Date(2022,2,10,0)) { // March 8, 2023
        cur_vote = 2;
    };
    return cur_vote;
};
